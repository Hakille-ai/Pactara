export const API_BASE =
  process.env.NEXT_PUBLIC_OMNIA_API_URL?.replace(/\/$/, "") ?? "http://localhost:8080";
...
export async function omniaFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    throw new Error(data?.error ?? `OMNIA API error ${response.status}`);
  }
