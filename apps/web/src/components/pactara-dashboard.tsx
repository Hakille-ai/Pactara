...
function IdentityPanel({
  ...
  return (
    <div className="space-y-5">
      <PanelTitle icon={Fingerprint} title="Create PACTARA Identity" />
...
function PactPanel({
  ...
  async function signPact() {
    ...
    const signed = await runAction(
      () => pactaraFetch<Pact>(`/v1/pacts/${pact.id}/sign`, { method: "POST" }),
      "PACT signed."
    );
    if (signed) {
      setPact(signed);
...
function VerifyPanel({
  ...
  async function verify() {
    const result = await runAction(
      () => pactaraFetch<VerifyPactResponse>(`/v1/pacts/${pactId}/verify`, { method: "POST" }),
  ...
  async function revoke() {
    ...
    await runAction(
      () =>
        pactaraFetch(`/v1/pacts/${pactId}/revoke`, {
          method: "POST",
    ...
    <div className="space-y-5">
      ...
      {qr ? (
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <img src={qr} alt="PACTARA PACT QR code" className="h-64 w-64 rounded border border-ink/10 bg-white p-3" />
...
function ProofPanel({
  ...
  const [payloadJson, setPayloadJson] = useState(
    '{"issuer":"PACTARA field verifier","claim":"Batch origin inspected","confidence":0.98}'
  );
...
function WorldPanel({
  ...
  const [summary, setSummary] = useState("Climate disruption delays transport, raises settlement risk, and triggers an PACTARA response workflow.");
  ...
  async function runScenario() {
    ...
    const run = await runAction(
      () =>
        pactaraFetch<ScenarioRun>(`/v1/world/scenarios/${selectedScenarioId}/run`, {
          method: "POST",
          body: JSON.stringify({
            parameters: {
              shock,
    ...
    if (created) {
      ...
      if (created.pact_id) {
        const pact = await runAction(
          () => pactaraFetch<Pact>(`/v1/pacts/${created.pact_id}`),
...
function TimelinePanel({
  ...
  async function refreshTimeline() {
    const loaded = await runAction(
      () => pactaraFetch<RuntimeTimelineItem[]>("/v1/runtime/timeline?limit=100"),
      "Timeline refreshed."
    );
    if (loaded) {
      setItems(loaded);
...
function WorkflowsPanel({
  ...
  async function loadWorkflow(id = selectedWorkflow) {
    if (!id) return;
    const response = await runAction(
      () => pactaraFetch<WorkflowResponse>(`/v1/workflows/${id}`),
      "Workflow loaded."
    );
    if (response) {
      setWorkflowResult(response);
...
function StreamPanel({
  ...
  async function readStream() {
    await runAction(async () => {
      ...
      if (!response.ok) {
        throw new Error(text || `PACTARA stream error ${response.status}`);
      }
...
function DomainsPanel({
  ...
  const [payloadJson, setPayloadJson] = useState(
    '{"asset":"PACT","amount":250,"purpose":"civilization operating layer"}'
  );
...
function LedgerPanel({
  ...
  async function loadStatement(accountId = selectedAccount) {
    ...
    const loaded = await runAction(
      () => pactaraFetch<LedgerStatement>(`/v1/ledger/accounts/${accountId}/statement?limit=40`),
  ...
  async function issueSandboxToken() {
    ...
    const issued = await runAction(
      () =>
        pactaraFetch<TokenIssuanceResponse>("/v1/token/issue", {
          method: "POST",
          body: JSON.stringify({
            issuer: identity.id,
  ...
  async function createHold() {
    ...
    const created = await runAction(
      () =>
        pactaraFetch<LedgerHold>("/v1/ledger/holds", {
          method: "POST",
          body: JSON.stringify({
            account_id: selectedAccount,
            pact_id: pact.id,
  ...
  async function releaseHold() {
    ...
    const released = await runAction(
      () => pactaraFetch<LedgerHold>(`/v1/ledger/holds/${hold.id}/release`, { method: "POST" }),
  ...
  async function createTransfer() {
    ...
    const response = await runAction(
      () =>
        pactaraFetch<LedgerTransferResponse>("/v1/ledger/transfers", {
          method: "POST",
          body: JSON.stringify({
  ...
  return (
    <div className="space-y-5">
      <PanelTitle icon={BadgeCheck} title="PACTARA Ledger" />
      <div className="grid gap-4 sm:grid-cols-3">
        <Field label="Owner">
          <input className="input" value={identity?.id ?? ""} readOnly />
        </Field>
            ...
            <select className="input" value={selectedAccount} onChange={(event) => setSelectedAccount(event.target.value)}>
              ...
              {accounts.map((account) => (
                <option key={account.id} value={account.id}>
                  {account.label} - {account.balance} PACT
                </option>
              ))}
            </select>
          </Field>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {accounts.map((account) => (
          ...
          >
            ...
            <p className="mt-3 text-3xl font-semibold text-signal">{account.balance} PACT</p>
...
function PaymentsPanel({
  ...
  async function createIntent() {
    ...
    const created = await runAction(
      () =>
        pactaraFetch<PaymentIntent>("/v1/payments/intents", {
          method: "POST",
          body: JSON.stringify({
            ...
            pact_id: pact.id,
            amount,
            memo: "PACTARA v0.5 sandbox settlement",
          }),
        }),
  ...
  async function executeIntent() {
    ...
    const response = await runAction(
      () => pactaraFetch<ExecutePaymentResponse>(`/v1/payments/${payment.id}/execute`, { method: "POST" }),
  ...
  async function rejectIntent() {
    ...
    const response = await runAction(
      () =>
        pactaraFetch<ExecutePaymentResponse>(`/v1/payments/${payment.id}/reject`, {
          method: "POST",
...
function AgentsPanel({
    ...
    const agent = await runAction(
      () =>
        pactaraFetch<AgentProfile>("/v1/agents", {
          method: "POST",
          body: JSON.stringify({
            label: "PACTARA Civilization Agent",
            model: "pactara-policy-agent-v0.5",
  ...
  async function runAgent() {
    const response = await runAction(
      () =>
        pactaraFetch<AgentRunResponse>(`/v1/agents/${agentId}/runs`, {
          method: "POST",
          body: JSON.stringify({
  ...
  async function refreshRuns() {
    const [runs, loadedTasks] = await Promise.allSettled([
      runAction(() => pactaraFetch<AgentRun[]>("/v1/ops/agent-runs?limit=30"), "Agent runs refreshed."),
      runAction(() => pactaraFetch<AgentTask[]>("/v1/agent-tasks?limit=30"), "Agent tasks refreshed."),
    ]);
  ...
  async function createTask() {
    const response = await runAction(
      () =>
        pactaraFetch<AgentTaskResponse>("/v1/agent-tasks", {
          method: "POST",
          body: JSON.stringify({
            agent_id: agentId,
            mandate_id: mandateId,
  ...
  async function approveTask() {
    ...
    const response = await runAction(
      () =>
        pactaraFetch<AgentTask>(`/v1/agent-tasks/${taskId}/approve`, {
          method: "POST",
  ...
  async function runTask() {
    ...
    const response = await runAction(
      () => pactaraFetch<AgentTaskRunResponse>(`/v1/agent-tasks/${taskId}/run`, { method: "POST" }),
