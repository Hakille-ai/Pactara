...
pub fn canonicalize_value(value: &Value) -> String {
    match value {
        ...
        Value::String(v) => serde_json::to_string(v).expect("serializing a JSON string cannot fail"),
        Value::Array(items) => {
            let inner = items
                .iter()
...
fn canonicalize_object(map: &Map<String, Value>) -> String {
    ...
    let inner = keys
        .into_iter()
        .map(|key| {
            let encoded_key = serde_json::to_string(key).expect("serializing a JSON key cannot fail");
...
fn decode_fixed<const N: usize>(value: &str) -> Result<[u8; N], CryptoError> {
    ...
    bytes.try_into().map_err(|_| CryptoError::InvalidLength { expected: N, actual })
}
