use chrono::Utc;
use omnia_core::{CreateDomainActionRequest, CreatePactRequest, DomainModule};
...
fn default_capabilities(id: &str) -> Value {
    match id {
        "economy" => json!({"actions": ["trade.create", "payment.request"], "requires_payment": true}),
        "knowledge" => json!({"actions": ["knowledge.claim", "learning.path"], "privacy": "public_or_private"}),
        "health" => json!({"actions": ["consent.grant", "care.request"], "raw_medical_data": false}),
        ...
        "link" => json!({"actions": ["relationship.consent", "creative.license"], "surveillance": false}),
        "space" => json!({"actions": ["place.register", "resource.claim"], "geospatial": true}),
        "transport" => json!({"actions": ["shipment.create", "custody.transfer"], "tracking": true}),
        _ => json!({}),
    }
}
