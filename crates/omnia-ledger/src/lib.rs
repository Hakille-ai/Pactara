use omnia_core::{LedgerAccount, Pact, PactStatus};

pub const OMN_ASSET_ID: &str = "asset:omn";
pub const OMN_SYMBOL: &str = "OMN";
...
mod tests {
    ...
    fn payment_requires_signed_active_pact() {
        let pact = Pact {
            ...
            object: json!({}),
            target: "omnia:person:b".to_string(),
            terms: json!({}),
            consent: json!({}),
            proof: json!({}),
            created_at: Utc::now(),
    ...
    fn account(balance: i64) -> LedgerAccount {
        LedgerAccount {
            id: Uuid::new_v4(),
            owner: "omnia:person:a".to_string(),
            asset_id: OMN_ASSET_ID.to_string(),
