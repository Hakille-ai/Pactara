...
use omnia_agent::{evaluate_policy as evaluate_policy_decision, mandate_allows_agent_run};
use omnia_auth::{
...
use omnia_crypto::{generate_key_material, hash_value, sign_value, verify_value};
use omnia_db::{Db, DbError};
use omnia_domain::action_to_pact_request;
use omnia_ledger::OMN_ASSET_ID;
use omnia_verifier::verify_bundle as verify_bundle_portable;
...
async fn get_pact_timeline(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
) -> Result<Json<Vec<omnia_core::EventLog>>, ApiError> {
...
) -> Result<Json<omnia_core::Revocation>, ApiError> {
...
async fn network_status(
    State(state): State<AppState>,
) -> Result<Json<omnia_core::NetworkStats>, ApiError> {
...
async fn create_domain_action(
    ...
    let default_terms = selected_template
        ...
        .unwrap_or_else(|| json!({"execution": "omnia-domain"}));
...
async fn create_dev_session(
    ...
    let session = AuthSession {
        id: Uuid::new_v4(),
        identity_id: payload.identity_id,
        session_token: format!("omnia_dev_{}", Uuid::new_v4()),
...
async fn create_ledger_account(
    ...
    let asset_id = payload.asset_id.unwrap_or_else(|| OMN_ASSET_ID.to_string());
...
async fn reject_payment(
    ...
    let transfer = omnia_core::LedgerTransfer {
        id: Uuid::nil(),
...
async fn create_agent(
    ...
    let identity_id = if let Some(identity_id) = payload.identity_id {
        ...
        let identity = state
            .db
            .create_identity(Identity {
                id: format!("omnia:agent:{}", Uuid::new_v4()),
    ...
    Ok(Json(
        state
            ...
            .create_agent_profile(AgentProfile {
                ...
                model: payload
                    .model
                    .unwrap_or_else(|| "omnia-policy-agent-v0.5".to_string()),
...
async fn create_agent_run(
    ...
    let pact_id = if let Some(pact_id) = payload.pact_id {
        ...
        let mut pact = Pact::new(CreatePactRequest {
            ...
            proof: json!({"agent_run": true, "protocol": "OMNIA v0.5"}),
            expires_at: None,
        });
        pact.hash = hash_value(&pact.signing_payload());
...
async fn create_agent_task(
    ...
    let log = state
        .db
        .create_agent_run_log(omnia_core::AgentRunLog {
            id: Uuid::new_v4(),
...
async fn create_workflow(
    ...
    let mut pact = Pact::new(CreatePactRequest {
        ...
        terms: json!({"runtime": "OMNIA v0.7", "risk": risk.risk_level.clone()}),
...
async fn review_workflow(
    ...
    Ok(Json(
        state
            .db
            .create_workflow_review(omnia_core::WorkflowReview {
                id: Uuid::new_v4(),
                workflow_id: id,
...
fn risk_from_workflow(payload: &CreateWorkflowRequest) -> omnia_core::RiskAssessment {
    let amount = payload
        .payload
        .get("amount")
...
async fn run_world_scenario(
    ...
    let risk = state
        .db
        .create_risk_assessment(omnia_core::RiskAssessment {
            id: Uuid::new_v4(),
            subject_id: scenario.id.to_string(),
            risk_level: risk_level.to_string(),
            score: impact_score,
    ...
    let pact = create_signed_runtime_pact(
        ...
        json!({"runtime": "OMNIA v0.8", "risk_level": risk_level, "impact_score": impact_score}),
...
async fn create_runtime_command(
    ...
    let pact = create_signed_runtime_pact(
        ...
        json!({"runtime": "OMNIA v0.8", "risk": risk.risk_level.clone()}),
...
async fn ensure_domain_exists(state: &AppState, domain_id: &str) -> Result<(), ApiError> {
    ...
    if exists {
        ...
        Err(ApiError::bad_request("unknown OMNIA domain"))
    }
}
...
fn risk_from_command(payload: &CreateRuntimeCommandRequest) -> omnia_core::RiskAssessment {
    let amount = payload
        .payload
        .get("amount")
...
async fn ensure_auth(state: &AppState, headers: &HeaderMap) -> Result<(), ApiError> {
        ...
        .is_some_and(|value| value == "true" || value == "1")
    {
        ...
        Err(ApiError::unauthorized(
            "OMNIA authentication is required in this environment",
        ))
    }
}
...
impl From<omnia_crypto::CryptoError> for ApiError {
    fn from(value: omnia_crypto::CryptoError) -> Self {
        Self {
            status: StatusCode::BAD_REQUEST,
            message: value.to_string(),
        }
...
mod tests {
    ...
    fn rejects_blank_runtime_command_fields() {
        let request = CreateRuntimeCommandRequest {
            actor: "omnia:person:a".to_string(),
            domain_id: "economy".to_string(),
            intent: "settle".to_string(),
            target: "omnia:org:b".to_string(),
