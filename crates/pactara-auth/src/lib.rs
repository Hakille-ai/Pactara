...
use pactara_core::{AuthChallenge, SignedRequest, SignedRequestVerification};
use pactara_crypto::{hash_value, verify_value};
...
mod tests {
    use super::*;
    use pactara_core::SignedRequest;
    use pactara_crypto::{generate_key_material, sign_value};
    use serde_json::json;
    ...
    fn expired_challenge_is_not_usable() {
        let challenge = AuthChallenge {
            ...
            ..create_challenge(Some("pactara:person:a".to_string()), "login", 60)
        };
    ...
    fn signed_request_rejects_replayed_nonce() {
        ...
        let mut request = SignedRequest {
            identity_id: "pactara:person:a".to_string(),
