use chrono::Utc;
use pactara_core::{Mandate, PolicyDecision, PolicyEvaluateRequest};
...
pub fn evaluate_policy(request: PolicyEvaluateRequest) -> PolicyDecision {
    ...
    let decision = if action.contains("sign_contract")
        || request.context.get("sensitivity").and_then(Value::as_str) == Some("medical_raw")
    {
        reasons.push("action requires stronger human authorization".to_string());
        "deny"
    } else if request.context.get("risk").and_then(Value::as_str) == Some("high")
        || request
            .context
            .get("amount")
...
mod tests {
    ...
    fn policy_can_allow_deny_or_need_review() {
        ...
        assert_eq!(
            decision("compare_prices", json!({"risk": "high"})),
            "needs_review"
        );
    }
    ...
    fn decision(action: &str, context: Value) -> String {
        evaluate_policy(PolicyEvaluateRequest {
            ...
            resource: "pactara:test".to_string(),
            context,
        })
        .decision
    }
